...
class Spider(object):
        ...
        for i in range(3, 9):
            ...
            for d in range(1, 39):
                ...
                for sc in team_list_score:
                    ...
                    base_url_cn = 'http://bf.win007.com/Count/{}cn.htm'.format(sid)
    ...
    def player(self):
        ...
        headers = {
            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/73.0.3683.86 Safari/537.36',
            'Referer': 'http://zq.win007.com/cn/TeamHeadPage/2018-2019/36.html',
            # 'Cookie': ' UM_distinctid=16a4e92d0c388-0a8e39accabd4d-5a40201d-1fa400-16a4e92d0c4116; detailCookie=null; CNZZDATA1261430177=299713168-1556095567-http%253A%252F%252Fwww.win007.com%252F%7C1556154968'
        }
        url_list='http://zq.win007.com/jsData/teamInfo/team36.js?version={}'
        url_list = url_list.format(self.now)

        response_list=requests.get(url=url_list,headers=headers)
        ...
        for num_s in team_list_score:
            troops.append(num_s[0])
        # print(troops)

        for i in troops:
            id = i
            base_url='http://zq.win007.com/jsData/teamInfo/teamDetail/tdl{}.js?version={}'
            # base_url = 'http://zq.win007.com/jsData/teamInfo/teamDetail/tdl348.js?version={}'
            ...
            # team_list_score = self.spiderman(response,pattern)
            list_str = list_str + ']'
            ...
            #
            # print(type(name), type(time), type(city), type(home), type(address), type(style_list), type(score_data_list))
    ...
    def situation(self, base_url_cn):
        mysqldb = DB()
        # base_url = 'http://bf.win007.com/Count/1552471cn.htm'
        # for i,j in self.troops.items():
        url = base_url_cn
        response = requests.get(url=url, headers=self.headers)
        html = etree.HTML(response.text)
        # html_list = html.xpath('//div[@class="content"][3]/table')
        # 1
        ...
        for n in range(3, 21):
            ...
            insert_sql = 'INSERT INTO qiutan_people(team_name, num, `name`, `position`, sm, sz, cq, dr, cqgr, cqc,' \
                         'cg, hq, zd, stjq, pf, sj)' \
